...
void AddSample(int r, int c, double rpos, double cpos, double rx, double cx, int step);
void PlaceInIndex(double dx, int ori1, double dy, int ori2, double rx, double cx);
void normalise();
void createLookups();
...
void initializeGlobals(Image *im, bool dbl = false, int insi = 4) {
    ...
    for (int i = 0; i < _height; ++i) {
        ...
        for (int j = 0; j < _width; ++j) {
            _Pixels[i][j] = static_cast<double>(tempPixels[i][j]);
        }
    }
    ...
    // allocate _index
    _index.resize(_IndexSize, std::vector<std::vector<double>>(_IndexSize, std::vector<double>(_OriSize, 0.0f)));
